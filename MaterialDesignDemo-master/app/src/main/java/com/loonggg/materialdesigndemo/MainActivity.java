...
public class MainActivity extends AppCompatActivity implements NavigationView
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPreferences = getSharedPreferences("loonggg", this.MODE_PRIVATE);
        //toolbar的设置，稍后讲这个控件
        ...
        adapter.setOnItemClickListener(new RecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View itemView, int pos) {
                final Snackbar snackbar = Snackbar.make(mRecyclerView, "你知道非著名程序员这个公众号吗？", Snackbar
                        .LENGTH_LONG);
                // 设置动作按钮颜色
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_home) {
            ...
            isNight = mPreferences.getBoolean("night", false);
            if (isNight) {
                AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_NO);
                mPreferences.edit().putBoolean("night", false).commit();
            } else {
                AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_YES);
                mPreferences.edit().putBoolean("night", true).commit();
            }
            recreate();
            return true;
        }
